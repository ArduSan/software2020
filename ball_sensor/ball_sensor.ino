...
void setup() {
  Serial.begin(57600);// velocità proiettazione  a monitor seriale
...
void loop() {
  ...
  byte txdata=0;
  txdata=index;
  if(SMax>50) txdata|=0b10000000;  // alzo il flag di palla in vista
  Serial.write(txdata);  // trasmetto il numero di sensore che ha il valore maggiore
